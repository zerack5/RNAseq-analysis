...
data2as <- read.csv('exp2as.csv', sep = ',',header = T,row.names = 1)
sdata2as <- data2as[c(grep("CD4",colnames(data2as)))]

data2c <- read.csv('exp2control.csv', sep = ',',header = T,row.names = 1)
...
##载入样本信息
fzdata2 <- read.csv('注释信息2.csv', sep = ',',header = T,row.names = 1)

sfzdata2 <- fzdata2[c(grep("CD4",fzdata2$type)),]
...
dds <-  DESeqDataSetFromMatrix(countData = count,colData = sfzdata2,design = ~batch)
dim(dds)
...
write.csv(diff,"all_diffcd4.csv")
