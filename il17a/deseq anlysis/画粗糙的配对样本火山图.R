...
heat1data<-read.csv("D:/数据分析中间文件/il17a/all_diff12_paired_res.csv", header = TRUE)
library(biomaRt)
...
t3g <- t2g[,c("ext_gene","ens_gene")]

heatdata <- merge(heat1data,t3g,by="ens_gene")


colnames(heatdata)
heatdata <- heatdata[,c("ext_gene","log2FoldChange","pvalue","padj" )]
...
data$selectedgene <- ifelse(data$padj<0.001 & abs(data$log2FoldChange)>1,data$ext_gene,NA)
head(data)
...
ggplot(data,aes(log2FoldChange,-log10(padj),
  ...
  datat <- na.omit(data$selectedgene)
...
ggsave("control and BL.tiff",device = "tiff",scale = 2,width = NA,height = NA,dpi = 300,path = "D:\\data anlysis\\pf\\figure")
